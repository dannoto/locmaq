...
//Formulário Perfuratriz
export default function FormPerfuratriz({ navigation, route }) {

    const {subnome, subkey, catkey} = route.params;
    navigation.setOptions({headerTitle: subnome.toUpperCase()});
    const navegacao = useNavigation();
    const categoria = {key:catkey, nome:'Perfuratriz'};
    ...
    function handleValidacao(){
        if (condicao.length < 1) {       
        ...
        else if (perfuracao.length < 1) {     
            errors.perfuracao = Alert.alert('Opps!', 'Informe a Perfuração.')
        }  
            ...
            navegacao.navigate('SegundoFormPerfuratriz', 
            { 
                condicao: condicao,
