// Segunda parte do Formulário Perfuratriz
...
export default ({route, navigation}) => {
    ...
    const {condicao, fabricante, modelo, ano, caracteristica, perfuracao, peso, potencia, seguro, infoAdicionais, subcategoria, categoria} = route.params;
    navigation.setOptions({headerTitle: subcategoria.nome.toUpperCase()});
    const navegacao = useNavigation();
    const { cadastrarPerfuratriz, user } = useContext(AuthContext);   
    ...
    function handleRegister() {
        if (estado.length < 1) {            
            ...
            if (imagens.length == 1) {
                ...
                if (cadastrarPerfuratriz (condicao, fabricante, modelo, ano, caracteristica, perfuracao, peso, potencia, seguro, infoAdicionais, estado, cidade, preco,
