// Segunda parte do Formulário Compactadores
...
export default ({route, navigation}) => {
    ...
    const {condicao, fabricante, modelo, ano, caracteristica, tipo, motorizacao, consumo, peso, horimetro, potencia, seguro, infoAdicionais, subcategoria, categoria} = route.params;
    navigation.setOptions({headerTitle: subcategoria.nome.toUpperCase()});
    const navegacao = useNavigation();
    const { cadastrarCompactador, user } = useContext(AuthContext);   
    ...
    function handleRegister() {
        if (estado.length < 1) {            
            ...
            if (imagens.length == 1) {
                ...
                if (cadastrarCompactador (condicao, fabricante, modelo, ano, caracteristica, tipo, motorizacao, consumo, peso, horimetro, potencia, seguro, infoAdicionais, estado, 
                    cidade, preco, precoHora, usuario, subcategoria, categoria, codigoProduto, imagem0, imagem1)) 
                { 
