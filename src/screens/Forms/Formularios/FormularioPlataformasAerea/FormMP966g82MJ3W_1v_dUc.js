...
export default function FormPlataformasAerea({ navigation, route }) {

    const {subnome, subkey, catkey} = route.params;
    navigation.setOptions({headerTitle: subnome.toUpperCase()});
    const navegacao = useNavigation();
    const categoria = {key:catkey, nome:'Plataformas Aérea'};
    ...
    function handleValidacao(){
        if (condicao.length < 1) {       
            ...
            errors.altura = Alert.alert('Opps!', 'Informe o Altura de Trabalho.')
        }    
            ...
            navegacao.navigate('SegundoFormPlataforma', 
            { 
            condicao: condicao,
