// Segunda parte do Formulário Plataformas Aerea
...
export default ({route, navigation}) => {
    ...
    const {condicao, fabricante, modelo, ano, caracteristica, capacidade, altura, potencia, seguro, infoAdicionais, subcategoria, categoria} = route.params;
    navigation.setOptions({headerTitle: subcategoria.nome.toUpperCase()});
    const navegacao = useNavigation();
    const { cadastrarPlataforma, user } = useContext(AuthContext);   
    ...
    function handleRegister() {
        if (estado.length < 1) {            
            ...
            if (imagens.length == 1) {
                ...
                if (cadastrarPlataforma (condicao, fabricante, modelo, ano, caracteristica, capacidade, altura, potencia, seguro, infoAdicionais, estado, cidade,  
