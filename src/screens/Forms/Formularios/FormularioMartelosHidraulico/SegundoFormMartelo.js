// Segunda parte do Formulário Martelos Hidraúlico
...
export default ({route, navigation}) => {
    ...
    const {condicao, fabricante, modelo, ano, peso, seguro, infoAdicionais, subcategoria, categoria} = route.params;
    navigation.setOptions({headerTitle: subcategoria.nome.toUpperCase()});
    const navegacao = useNavigation();
    const { cadastrarMartelo, user } = useContext(AuthContext);   
    ...
    function handleRegister() {
        if (estado.length < 1) {            
            ...
            if (imagens.length == 1) {
                ...
                if (cadastrarMartelo (condicao, fabricante, modelo, ano, peso, seguro, infoAdicionais, estado, cidade, preco, precoHora, 
                    usuario, subcategoria, categoria, codigoProduto, imagem0, imagem1)) 
                { 
