// Segunda parte do Formulário Manipuladores Telescópico
...
export default ({route, navigation}) => {
    ...
    const {condicao, fabricante, modelo, ano, consumo, peso, lanca, horimetro, potencia, seguro, infoAdicionais, subcategoria, categoria} = route.params;
    navigation.setOptions({headerTitle: subcategoria.nome.toUpperCase()});
    const navegacao = useNavigation();
    const { cadastrarManipulador, user } = useContext(AuthContext);   
    ...
    function handleRegister() {
        if (estado.length < 1) {            
            ...
            if (imagens.length == 1) {
                ...
                if (cadastrarManipulador (condicao, fabricante, modelo, ano, consumo, peso, lanca, horimetro, potencia, seguro, infoAdicionais, estado, cidade, preco,
