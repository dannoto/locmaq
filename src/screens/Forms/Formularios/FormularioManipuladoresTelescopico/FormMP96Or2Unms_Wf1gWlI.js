...
export default function FormManipuladoresTelescópico({ navigation, route }) {

    const {subnome, subkey, catkey} = route.params;
    navigation.setOptions({headerTitle: subnome.toUpperCase()});
    const navegacao = useNavigation();
    const categoria = {key:catkey, nome:'Manipuladores Telescópico'};
    ...
    function handleValidacao(){
        if (condicao.length < 1) {       
            ...
            navegacao.navigate('SegundoFormManipulador', 
            { 
                condicao: condicao,
