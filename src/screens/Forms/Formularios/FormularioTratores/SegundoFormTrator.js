// Segunda parte do Formulário Tratores
...
export default ({route, navigation}) => {
    ...
    const {condicao, fabricante, modelo, ano, consumo, peso, horimetro, potencia, seguro, infoAdicionais, subcategoria, categoria} = route.params;
    navigation.setOptions({headerTitle: subcategoria.nome.toUpperCase()});
    const navegacao = useNavigation();
    const { cadastrarTrator, user } = useContext(AuthContext);   
    ...
    function handleRegister() {
        if (estado.length < 1) {            
            ...
            if (imagens.length == 1) {
                ...
                if (cadastrarTrator (condicao, fabricante, modelo, ano, consumo, peso, horimetro, potencia, seguro, infoAdicionais, estado, cidade, preco, 
