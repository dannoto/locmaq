// Segunda parte do Formulário Empilhadeiras
...
export default ({route, navigation}) => {
    ...
    const {condicao, fabricante, modelo, ano, caracteristica, tipo, capacidade, altura, peso, potencia, seguro, infoAdicionais, subcategoria, categoria} = route.params;
    navigation.setOptions({headerTitle: subcategoria.nome.toUpperCase()});
    const navegacao = useNavigation();
    const { cadastrarEmpilhadeira, user } = useContext(AuthContext);   
    ...
    const [precoHora, setPrecoDiaria] = useState('');
    ...
    function handleRegister() {
        if (estado.length < 1) {            
            ...
            if (imagens.length == 1) {
                ...
                if (cadastrarEmpilhadeira (condicao, fabricante, modelo, ano, caracteristica, tipo, capacidade, altura, peso, potencia, seguro, infoAdicionais, estado, cidade,
