...
export default function FormEmpilhadeiras({ navigation, route }) {
    
    const {subnome, subkey, catkey} = route.params;
    navigation.setOptions({headerTitle: subnome.toUpperCase()});
    const navegacao = useNavigation();
    const categoria = {key:catkey, nome:'Empilhadeiras'};
    ...
    function handleValidacao(){
        if (condicao.length < 1) {       
            ...
            errors.capacidade = Alert.alert('Opps!', 'Informe a Capacidade Máxima.')
        } 
        else if (altura.length < 1) {     
            errors.altura = Alert.alert('Opps!', 'Informe a Altura Máxima.')
        } 
            ...
            navegacao.navigate('SegundoFormEmpilhadeira', 
            { 
                condicao: condicao,
