// Segunda parte do Formulário Guindastes
...
export default ({route, navigation}) => {
    ...
    const {condicao, fabricante, modelo, ano, caracteristica, capacidade, lanca, potencia, seguro, infoAdicionais, subcategoria, categoria} = route.params;
    navigation.setOptions({headerTitle: subcategoria.nome.toUpperCase()});
    const navegacao = useNavigation();
    const { cadastrarGuindaste, user } = useContext(AuthContext);   
    ...
    function handleRegister() {
        if (estado.length < 1) {            
            ...
            if (imagens.length == 1) {
                ...
                if (cadastrarGuindaste (condicao, fabricante, modelo, ano, caracteristica, capacidade, lanca, potencia, seguro, infoAdicionais, estado, cidade, preco, 
