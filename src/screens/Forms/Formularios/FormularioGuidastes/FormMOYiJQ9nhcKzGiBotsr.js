...
//Formulário Guindastes
export default function FormGuindastes({ navigation, route }) {

    const {subnome, subkey, catkey} = route.params;
    navigation.setOptions({headerTitle: subnome.toUpperCase()});
    const navegacao = useNavigation();
    const categoria = {key:catkey, nome:'Guindastes'};
    ...
    function handleValidacao(){
        if (condicao.length < 1) {       
            ...
            errors.capacidade = Alert.alert('Opps!', 'Informe a Capacidade de Carga.')
        }  
        else if (lanca.length < 1) {                   
            errors.lanca = Alert.alert('Opps!', 'Informe o Alcance da Lança.')
        } 
            ...
            navegacao.navigate('SegundoFormGuindaste', 
            { 
                condicao: condicao,
