...
//Formulário Usinas de Concreto
export default function FormUsinasdeConcreto({ navigation, route }) {

    const {subnome, subkey, catkey} = route.params;
    navigation.setOptions({headerTitle: subnome.toUpperCase()});
    const navegacao = useNavigation();
    const categoria = {key:catkey, nome:'Usinas de Concreto'};
    ...
    function handleValidacao(){
        if (condicao.length < 1) {       
            ...
            navegacao.navigate('SegundoFormUsinaConcreto', 
            { 
                condicao: condicao,
