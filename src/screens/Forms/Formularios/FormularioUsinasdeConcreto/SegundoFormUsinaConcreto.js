// Segunda parte do Formulário Usinas de Concreto
...
export default ({route, navigation}) => {
    ...
    const {condicao, fabricante, modelo, ano, caracteristica, tipo, capacidade, peso, potencia, seguro, infoAdicionais, subcategoria, categoria} = route.params;
    navigation.setOptions({headerTitle: subcategoria.nome.toUpperCase()});
    const navegacao = useNavigation();
    const { cadastrarUsinaConcreto, user } = useContext(AuthContext);   
    ...
    function handleRegister() {
        if (estado.length < 1) {            
            ...
            if (imagens.length == 1) {
                ...
                if (cadastrarUsinaConcreto (condicao, fabricante, ano, modelo, caracteristica, tipo, capacidade, peso, potencia, seguro, infoAdicionais, estado, cidade, preco,
                    precoHora, usuario, subcategoria, categoria, codigoProduto, imagem0, imagem1)) 
                { 
