...
export default function FormEditPerfuratriz({data}) {
    ...
    const [opcoescaracteristica, setOpcoesCaracteristica] = useState([
        {key: 0, nome: 'SELECIONAR'}, 
        {key: 1, nome: 'AUTOPROPELIDO'}, 
        {key: 2, nome: 'CAMINHÃO'},
        {key: 3, nome: 'IMPLEMENTO'}, 
        {key: 4, nome: 'GIRATÓRIA'},
        {key: 5, nome: 'ROTOPERCUSSIVA'}
    ]);
    const [perfuracao, setPerfuracao] = useState('');
    const [perfuracaoOption, setPerfuracaoOption] = useState([
        {key: 0, nome: 'SELECIONAR'}, 
        {key: 1, nome: 'HORIZONTAL'}, 
        {key: 2, nome: 'VERTICAL'}
    ]);
    ...
    const [ModalPerfuracao, setModalPerfuracao] = useState(false);
...
//Funções Update no Firebase
    ...
    async function updatePerfuracao() {
        await firebase.database().ref('equipamentos').child(data).update({
            perfuracao: perfuracao
        })
        setModalPerfuracao(false)
        setPerfuracao()
    }
    ...
    let perfuracaoItem = perfuracaoOption.map( (v, k) => {
        return <Picker.Item key={k} value={v} label={v.nome}/>
    })
    ...
    function SelectPadraoPerfuracao(v,i) {
        if (v.key !== 0) {
           setPerfuracao(v);
        }  
    }
    ...
    return (
        <ScrollView style={[styles.background, modalvisible || modalCondicao || modalFabricante || modalModelo || modalAno || modalCaracteristica ||  
            ModalPerfuracao || modalPeso || modalPotencia || modalSeguro || modalinfoAdicionais || modalEstado || modalCidade || modalPreco || modalPrecoHora
         ? {backgroundColor: '#fff', opacity: 0.1} : '']}>
            ...
            enabled>
                ...
                <View style={styles.areaEdicao}>
                    <View>
                        <Text style={styles.tituloInput}>PERFURAÇÃO</Text>
                        <Text style={styles.dados}>{detalhes.perfuracao}</Text>
                    </View>

                    <TouchableOpacity onPress={() => setModalPerfuracao(true)}>
                        <MaterialIcons
                            style={styles.icon}
                            name='edit'
                            size= {28}
                    ...
                    <Modal animationType="fade" transparent={true} visible={ModalPerfuracao} onRequestClose={() => {}}>
                        <View style={styles.modalWindow}>
                            <View style={styles.modalBody}>
                                <TouchableOpacity style={styles.areaBtnModalClose} onPress={() => setModalPerfuracao(false)}>
                                    <AntDesign
                                    style={{marginBottom: 5}}
                                    name='closecircleo'
                                    size= {34}
                                ...
                                <View style={styles.picker}>
                                    <Picker
                                    selectedValue={perfuracao}
                                    onValueChange={(itemValue, itemIndex) => SelectPadraoPerfuracao(itemValue,itemIndex)}
                                    style= {{color: '#fff'}}
                                    dropdownIconColor={'white'}
                                    >
                                        {perfuracaoItem}
                                ...
                                <TouchableOpacity style={styles.areaBtnModal} onPress={updatePerfuracao}>
                                    <Text style={styles.txtBtnModal}>SALVAR ALTERAÇÕES</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
