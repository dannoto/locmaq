...
export default function FormEditCompactadores({data}) {
    ...
    const [modalMotorizacao, setModalMotorizacao] = useState(false);
...
//Funções Update no Firebase
    ...
    async function updateMotorizacao() {
        await firebase.database().ref('equipamentos').child(data).update({
            motorizacao: motorizacao
        })
        setModalMotorizacao(false)
        setMotorizacao()
    }
    ...
    return (
        <ScrollView style={[styles.background, modalvisible || modalCondicao || modalFabricante || modalModelo || modalAno || modalCaracteristica ||  modalTipo || modalMotorizacao || modalConsumo ||
            modalPeso || modalHorimetro || modalPotencia || modalSeguro || modalinfoAdicionais || modalEstado || modalCidade || modalPreco || modalPrecoHora
            ...
            enabled>
                ...
                <View style={styles.areaEdicao}>
                    <View>
                        ...
                        <Text style={styles.dados}>{detalhes.motorizacao}</Text>
                    </View>

                    <TouchableOpacity onPress={() => setModalMotorizacao(true)}>
                        <MaterialIcons
                            style={styles.icon}
                            name='edit'
                            size= {28}
                    ...
                    <Modal animationType="fade" transparent={true} visible={modalMotorizacao} onRequestClose={() => {}}>
                        <View style={styles.modalWindow}>
                            <View style={styles.modalBody}>
                                <TouchableOpacity style={styles.areaBtnModalClose} onPress={() => setModalMotorizacao(false)}>
                                    <AntDesign
                                    style={{marginBottom: 5}}
                                    name='closecircleo'
                                    size= {34}
                                ...
                                <TouchableOpacity style={styles.areaBtnModal} onPress={updateMotorizacao}>
                                    <Text style={styles.txtBtnModal}>SALVAR ALTERAÇÕES</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
