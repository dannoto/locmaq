...
export default function FormEditGuindastes({data}) {
    ...
    const [modalLanca, setModalLanca] = useState(false);
...
//Funções Update no Firebase
    ...
    return (
        <ScrollView style={[styles.background, modalvisible || modalCondicao || modalFabricante || modalModelo || modalAno || modalCaracteristica || 
            modalCapacidade || modalLanca || modalPotencia || modalSeguro || modalinfoAdicionais || modalEstado || modalCidade || modalPreco || modalPrecoHora
         ? {backgroundColor: '#fff', opacity: 0.1} : '']}>
            ...
            enabled>
                ...
                <View style={styles.areaEdicao}>
                    <View>
                        <Text style={styles.tituloInput}>CAPACIDADE DE CARGA (TON)</Text>
                ...
                <View style={styles.areaEdicao}>
                    <View>
                        <Text style={styles.tituloInput}>ALCANCE DA LANÇA (M)</Text>
                        <Text style={styles.dados}>{detalhes.lanca}</Text>
                    </View>

                    <TouchableOpacity onPress={() => setModalLanca(true)}>
                        <MaterialIcons
                            style={styles.icon}
                            name='edit'
                            size= {28}
                    ...
                    <Modal animationType="fade" transparent={true} visible={modalLanca} onRequestClose={() => {}}>
                        <View style={styles.modalWindow}>
                            <View style={styles.modalBody}>
                                <TouchableOpacity style={styles.areaBtnModalClose} onPress={() => setModalLanca(false)}>
                                    <AntDesign
                                    style={{marginBottom: 5}}
                                    name='closecircleo'
                                    size= {34}
                                ...
                                <View style={styles.areaInput}>
                                    <TextInput
                                        ...
                                        placeholderTextColor='#fff'
                                        value={lanca}
                                        onChangeText={(text) => setLanca(text)}
                                        keyboardType={'numeric'}
                                        maxLength={20}
                                    />
                                </View>

                                <TouchableOpacity style={styles.areaBtnModal} onPress={updateLanca}>
                                    <Text style={styles.txtBtnModal}>SALVAR ALTERAÇÕES</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
